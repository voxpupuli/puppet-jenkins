  ...
  if (empty(grep($installed_plugins, $search))) {
    ...
    if $::osfamily == 'windows' { 
	  pget { "${plugin}-${version}" :
        ...
        targetfilename => "${plugin}-${version}",
        notify         => Exec["copy_${plugin}"],
		require          => File[$::jenkins::plugin_dir],
      }
      exec { "copy_${plugin}":
        command     => "Copy-Item C:/windows/temp/${plugin}-${version} 'C:/Program Files (x86)/Jenkins/plugins/${plugin}' -Force -ErrorAction SilentlyContinue | out-null ",
        refreshonly => true,
        provider    => powershell,
        onlyif      => "if (test-path C:/windows/temp/${plugin}-${version}) {exit 0} else {exit 1}",
		notify => Service['jenkins'],
      }
    $require = Pget[$plugin]
    } else { 
