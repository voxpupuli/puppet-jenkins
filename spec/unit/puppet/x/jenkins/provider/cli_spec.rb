...
describe Puppet::X::Jenkins::Provider::Cli do
  ...
  describe '::prefetch' do
    ...
    it 'does not break an existing resource/provider association' do
      ...
      expect(described_class).to receive(:instances).with(catalog) { [provider] }
  ...
  describe '#cli' do
    ...
    it 'extracts the catalog from the resource' do
      ...
      expect(described_class).to receive(:cli).with('foo', catalog: catalog)
      ...
      context 'without ssh_private_key' do
        CLI_NET_ERRORS.each do |error|
          it 'retries cli on NetError exception' do
            ...
            expect { described_class.cli('foo') }.to raise_error(Puppet::Util::RetryAction::RetryException::RetriesExceeded)
          end
        end
      end
      # without ssh_private_key
    ...
    context 'when UnknownError exception' do
      ...
      context 'retry n times' do
        it 'by default' do
          ...
          expect { described_class.cli('foo', catalog: catalog) }.to raise_error(UnknownError, 'foo')
        end
