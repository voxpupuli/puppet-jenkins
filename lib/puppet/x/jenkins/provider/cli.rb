...
class Puppet::X::Jenkins::Provider::Cli < Puppet::Provider
  ...
  class_variable_set(:@@cli_auth_required, false) # rubocop:todo Style/ClassVars
  ...
  def self.cli(command, options = {}, cli_pre_cmd = [])
    return execute_with_retry(command, options, cli_pre_cmd) if options.nil? || (!options.key?(:stdinjson) && !options.key?(:stdin))
  ...
  def self.execute_with_retry(command, options = {}, cli_pre_cmd = [])
    ...
    elsif !cli_username.nil? && !cli_password.nil? # rubocop:todo Lint/DuplicateBranch
      auth_cmd = base_cmd + ['-auth', "@#{cli_password_file}"] + [command]
    end
    auth_cmd&.flatten!
  ...
  def self.execute_with_auth(cli_cmd, auth_cmd, options = {})
    ...
    return execute_exceptionify(auth_cmd, options) if class_variable_get(:@@cli_auth_required)
    ...
    begin
      ...
      class_variable_set(:@@cli_auth_required, true) # rubocop:todo Style/ClassVars
      Puppet.debug("#{sname} cli_auth_required: #{class_variable_get(:@@cli_auth_required)}")
      result
    end
  end
  ...
  def self.execute_exceptionify(cmd, options)
    ...
    tmpfile_as_param = options[:tmpfile_as_param] if options.key?(:tmpfile_as_param)

    begin
      return superclass.execute([cmd, options[:stdinfile]].flatten.join(' '), options) if tmpfile_as_param && options.key?(:stdinfile)
